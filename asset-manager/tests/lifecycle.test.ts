...
describe("分配", () => {
  describe("allocateAssets — 从设备池分配闲置设备", () => {
    it("可以将闲置设备分配给员工", async () => {
      ...
      expect(updated?.employeeId).toBe(emp.data!.id);
      ...
      expect(logs[0].employeeId).toBe(emp.data!.id);
    ...
    it("可以批量分配多台设备给同一个员工", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.allocatedCount).toBe(3);
      ...
      expect(assets.every((a) => a.employeeId === emp.data!.id)).toBe(true);
    });
    ...
    it("只有闲置设备才能分配", async () => {
      ...
      expect(result.error).toContain("非闲置");
    });
...
describe("归还", () => {
  describe("returnAssets — 批量归还设备", () => {
    ...
    it("只有 IN_USE 状态的设备才能归还", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("非在用");
    });
...
describe("调拨", () => {
  describe("transferAssets — 批量调拨设备", () => {
    it("可以将设备从一个员工调拨给另一个员工", async () => {
      ...
      const updated = await prisma.asset.findUnique({ where: { id: asset.id } });
      expect(updated?.status).toBe("IN_USE");
      expect(updated?.employeeId).toBe(emp2.data!.id);

      const logs = await prisma.lifecycleLog.findMany({
        where: { assetId: asset.id, action: "TRANSFERRED" },
      });
      expect(logs).toHaveLength(1);
      expect(logs[0].fromEmployeeId).toBe(emp.data!.id);
      expect(logs[0].employeeId).toBe(emp2.data!.id);
    });
...
describe("升级", () => {
  describe("upgradeAssetComponent — 更换设备配件", () => {
    ...
    it("设备上没有该配件时升级失败", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("设备上不存在该配件");
    });
  });
});
...
describe("报废", () => {
  describe("scrapAssets — 批量报废设备", () => {
    ...
    it("已报废的设备不能重复报废", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("已报废");
    });
    ...
    it("批量报废多台设备", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.scrappedCount).toBe(2);
    });
