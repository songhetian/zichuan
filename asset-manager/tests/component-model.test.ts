...
describe("配件型号 CRUD", () => {
  describe("createComponentModel", () => {
    it("可以创建配件型号并自动初始化库存为 0", async () => {
      ...
      expect(result.data?.name).toBe("i7-12700F");
      expect(result.data?.brand).toBe("Intel");
      expect(result.data?.categoryId).toBe(cat.id);
  ...
  describe("getComponentModels", () => {
    ...
    it("可以按分类筛选", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.length).toBe(2);
      expect(result.data?.every((m) => m.categoryId === cat1.id)).toBe(true);
    });
  ...
  describe("getComponentModelById", () => {
    it("可以根据 ID 获取型号详情", async () => {
      ...
      const result = await getComponentModelById(created.data!.id);
  ...
  describe("updateComponentModel", () => {
    it("可以更新型号名称和品牌", async () => {
      ...
      const result = await updateComponentModel(created.data!.id, {
        name: "新型号",
        brand: "新品牌",
      });

      expect(result.success).toBe(true);
      expect(result.data?.name).toBe("新型号");
      expect(result.data?.brand).toBe("新品牌");
    });
    ...
    it("同一分类下更新名称不能与已有型号重复", async () => {
      ...
      const result = await updateComponentModel(target.data!.id, {
        name: "i5",
      });
    ...
    it("可以更改所属分类", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.categoryId).toBe(cat2.id);
    });
  ...
  describe("deleteComponentModel", () => {
    it("可以删除没有库存流水的型号", async () => {
      ...
      const result = await deleteComponentModel(created.data!.id);
