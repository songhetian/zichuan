...
describe("系统日志", () => {
  ...
  describe("getSystemLogs", () => {
    it("可以获取全部日志（按时间倒序）", async () => {
      ...
      expect(result.data![0].module).toBe("B"); // 最新的在前
      expect(result.data![1].module).toBe("A");
    });
    ...
    it("可以按模块筛选", async () => {
      ...
      expect(result.data?.length).toBe(2);
      expect(result.data?.every((l) => l.module === "分配")).toBe(true);
    });
    ...
    it("可以按关键词搜索详情", async () => {
      ...
      expect(result.data?.length).toBe(1);
      expect(result.data![0].detail).toContain("张三");
    });
  });
});
