...
describe("设备模板 CRUD", () => {
  describe("createDeviceTemplate", () => {
    it("可以创建带有 BOM 配件清单的设备模板", async () => {
      ...
      expect(result.data?.name).toBe("标准办公电脑");
      expect(result.data?.categoryId).toBe(assetCat.id);
    ...
    it("BOM 中包含不存在的配件型号时创建失败", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain("配件型号不存在");
    });
  ...
  describe("getDeviceTemplates", () => {
    it("可以获取全部模板列表（含 BOM 明细）", async () => {
      ...
      expect(result.data![0].components).toHaveLength(1);
    });
    ...
    it("可以按分类筛选", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.length).toBe(1);
      expect(result.data![0].name).toBe("电脑模板");
    });
  ...
  describe("getDeviceTemplateById", () => {
    it("可以根据 ID 获取模板详情（含 BOM）", async () => {
      ...
      const result = await getDeviceTemplateById(created.data!.id);
  ...
  describe("updateDeviceTemplate", () => {
    it("可以更新模板名称和 BOM", async () => {
      ...
      const result = await updateDeviceTemplate(created.data!.id, {
        name: "新模板",
        components: [
          { modelId: cpu.id, quantity: 1 },
          { modelId: ram.id, quantity: 2 },
    ...
    it("同一分类下更新名称不能与已有模板重复", async () => {
      ...
      const result = await updateDeviceTemplate(target.data!.id, {
        name: "模板A",
      });
  ...
  describe("deleteDeviceTemplate", () => {
    it("可以删除没有关联设备的模板", async () => {
      ...
      const result = await deleteDeviceTemplate(created.data!.id);
    ...
    it("删除模板时级联删除 BOM 配件清单", async () => {
      ...
      await deleteDeviceTemplate(created.data!.id);
    ...
    it("有关联设备时不能删除", async () => {
      ...
      const result = await deleteDeviceTemplate(template.data!.id);
