...
describe("库存盘点", () => {
  describe("createStocktakeSession", () => {
    it("可以创建盘点任务并自动生成盘点明细", async () => {
      ...
      expect(result.data?.name).toBe("月度盘点");
      expect(result.data?.status).toBe("OPEN");

      // 验证明细记录数量
      const records = await prisma.stocktakeRecord.findMany({
        where: { sessionId: result.data!.id },
      });
  ...
  describe("getStocktakeSessionById", () => {
    it("可以获取盘点任务详情（含明细）", async () => {
      ...
      const result = await getStocktakeSessionById(created.data!.id);
  ...
  describe("updateStocktakeRecord", () => {
    it("可以标记设备为盘亏", async () => {
      ...
      const records = await prisma.stocktakeRecord.findMany({
        where: { sessionId: session.data!.id },
      });
  ...
  describe("completeStocktakeSession", () => {
    it("可以完成盘点并统计盘盈盘亏", async () => {
      ...
      const result = await completeStocktakeSession(session.data!.id);
    ...
    it("未完成的任务不能重复完成", async () => {
      ...
      await completeStocktakeSession(session.data!.id);
