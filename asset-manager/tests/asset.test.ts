...
describe("设备实体 CRUD", () => {
  describe("createAsset", () => {
    it("可以按模板生成设备并自动分配编号", async () => {
      ...
      expect(result.data?.assetNo).toMatch(/^DN-\d{4}$/);
      expect(result.data?.name).toBe("张三的办公电脑");
      ...
      expect(result.data?.components[0].modelId).toBe(cpu.id);
      expect(result.data?.components[1].quantity).toBe(2);
    });
    ...
    it("多台设备编号自动递增", async () => {
      ...
      expect(r1.data?.assetNo).toBe("DN-0001");
      expect(r2.data?.assetNo).toBe("DN-0002");
    });
  ...
  describe("getAssets", () => {
    ...
    it("可以按状态筛选", async () => {
      ...
      expect(result.data![0].name).toBe("电脑2");
    });
    ...
    it("可以按分类筛选", async () => {
      ...
      expect(result.data?.length).toBe(1);
      expect(result.data![0].name).toBe("电脑");
    });
    ...
    it("可以按关键词搜索", async () => {
      ...
      expect(result.data?.length).toBe(1);
      expect(result.data![0].name).toBe("张三的电脑");
    });
  ...
  describe("getAssetById", () => {
    it("可以根据 ID 获取设备详情（含配置和日志）", async () => {
      ...
      const result = await getAssetById(created.data!.id);

      expect(result.success).toBe(true);
      expect(result.data?.name).toBe("电脑1");
      expect(result.data?.components).toHaveLength(2);
      expect(result.data?.lifecycleLogs).toHaveLength(1);
    });
  ...
  describe("updateAsset", () => {
    it("可以更新设备基本信息", async () => {
      ...
      const result = await updateAsset(created.data!.id, {
        name: "新名称",
        location: "办公室 A",
        notes: "备注信息",
      });
      ...
      expect(result.data?.location).toBe("办公室 A");
  ...
  describe("deleteAsset", () => {
    it("可以删除设备并级联删除配件配置和日志", async () => {
      ...
      const result = await deleteAsset(created.data!.id);

      expect(result.success).toBe(true);

      const asset = await prisma.asset.findUnique({ where: { id: created.data!.id } });
      expect(asset).toBeNull();

      const components = await prisma.assetComponent.findMany({
        where: { assetId: created.data!.id },
      });
