...
describe("配件库存出入库", () => {
  describe("purchaseStockIn（采购入库）", () => {
    it("采购入库增加库存数量并记录流水", async () => {
      ...
      expect(stock.data?.quantity).toBe(10);

      // 检查流水
      const logs = await getStockLogs({ modelId: model.id });
      expect(logs.success).toBe(true);
      expect(logs.data?.length).toBe(1);
      expect(logs.data![0].type).toBe("PURCHASE_IN");
      expect(logs.data![0].quantity).toBe(10);
      expect(logs.data![0].operator).toBe("admin");
      expect(logs.data![0].remark).toBe("首批采购");
    });
    ...
    it("多次入库数量累加", async () => {
      ...
      const stock = await getStockByModelId(model.id);
      expect(stock.data?.quantity).toBe(15);
    });
  });
  ...
  describe("upgradeReturnStockIn（升级退回入库）", () => {
    it("升级退回的旧配件回库并记录流水", async () => {
      ...
      const stock = await getStockByModelId(model.id);
      expect(stock.data?.quantity).toBe(11);

      const logs = await getStockLogs({ modelId: model.id });
      const returnLog = logs.data?.find((l) => l.type === "UPGRADE_RETURN");
  ...
  describe("assetBuildStockOut（组装设备出库）", () => {
    it("组装设备扣减库存并记录流水", async () => {
      ...
      const stock = await getStockByModelId(model.id);
      expect(stock.data?.quantity).toBe(7);

      const logs = await getStockLogs({ modelId: model.id });
      const outLog = logs.data?.find((l) => l.type === "ASSET_BUILD");
  ...
  describe("upgradeUseStockOut（升级使用出库）", () => {
    it("升级使用新配件扣减库存并记录流水", async () => {
      ...
      const logs = await getStockLogs({ modelId: model.id });
      const outLog = logs.data?.find((l) => l.type === "UPGRADE_USE");
      expect(outLog).toBeDefined();
  ...
  describe("getStockLogs（库存流水查询）", () => {
    it("可以按型号筛选流水", async () => {
      ...
      await purchaseStockIn({ modelId: model2.data!.id, quantity: 20, operator: "admin" });

      const logs = await getStockLogs({ modelId: model1.id });
      expect(logs.success).toBe(true);
      expect(logs.data?.length).toBe(1);
      expect(logs.data![0].modelId).toBe(model1.id);
    });
    ...
    it("可以按操作类型筛选", async () => {
      ...
      expect(inLogs.data?.length).toBe(1);
      expect(inLogs.data![0].type).toBe("PURCHASE_IN");
    });
    ...
    it("流水按时间倒序排列", async () => {
      ...
      // 最新的在前，数量为 5 的是后入库的
      expect(logs.data![0].quantity).toBe(5);
      expect(logs.data![1].quantity).toBe(10);
    });
  ...
  describe("批量出入库（事务性）", () => {
    it("批量出库时如果任何一个库存不足，全部回滚", async () => {
      ...
      const ram = await createComponentModel({
        name: "16GB",
        categoryId: cat.data!.id, // 先凑合，实际分类不同
      });

      await purchaseStockIn({ modelId: cpu.data!.id, quantity: 10, operator: "admin" });
      await purchaseStockIn({ modelId: ram.data!.id, quantity: 1, operator: "admin" }); // 只有 1 条内存

      // 模拟组装 3 台设备，需要 3 条内存，但只有 1 条
      // 直接调用事务性的批量出库函数
      const result = await batchStockOut([
        { modelId: cpu.data!.id, quantity: 3, type: "ASSET_BUILD" as const },
        { modelId: ram.data!.id, quantity: 3, type: "ASSET_BUILD" as const },
      ], "admin", "组装设备");
      ...
      const cpuStock = await getStockByModelId(cpu.data!.id);
      expect(cpuStock.data?.quantity).toBe(10);
    });
  });
});
