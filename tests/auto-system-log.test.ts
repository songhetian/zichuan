...
describe("系统日志自动记录", () => {
  ...
  describe("分配操作自动记录系统日志", () => {
    it("分配设备后自动生成系统日志", async () => {
      ...
      expect(unwrap(logs).length).toBeGreaterThanOrEqual(1);
      const allocLog = unwrap(logs).find((l) => l.detail.includes(asset.assetNo));
  ...
  describe("归还操作自动记录系统日志", () => {
    it("归还设备后自动生成系统日志", async () => {
      ...
      expect(logs.success).toBe(true);
      const returnLog = unwrap(logs).find((l) => l.detail.includes(asset.assetNo));
  ...
  describe("调拨操作自动记录系统日志", () => {
    it("调拨设备后自动生成系统日志", async () => {
      ...
      expect(logs.success).toBe(true);
      const transferLog = unwrap(logs).find((l) => l.detail.includes(asset.assetNo));
  ...
  describe("报废操作自动记录系统日志", () => {
    it("报废设备后自动生成系统日志", async () => {
      ...
      expect(logs.success).toBe(true);
      const scrapLog = unwrap(logs).find((l) => l.detail.includes(asset.assetNo));
  ...
  describe("升级操作自动记录系统日志", () => {
    it("升级设备配件后自动生成系统日志", async () => {
      ...
      expect(logs.success).toBe(true);
      const upgradeLog = unwrap(logs).find((l) => l.detail.includes(asset.assetNo));
  ...
  describe("入库操作自动记录系统日志", () => {
    it("采购入库后自动生成系统日志", async () => {
      ...
      expect(logs.success).toBe(true);
      expect(unwrap(logs).length).toBeGreaterThanOrEqual(1);
      const stockLog = unwrap(logs).find((l) => l.detail.includes("16GB DDR4"));
