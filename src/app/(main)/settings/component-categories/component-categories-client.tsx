...
export function ComponentCategoriesClient({
  ...
  const handleDragEnd = async (draggedId: number, targetId: number, position: 'before' | 'after' | 'inside') => {
    if (position === 'inside') {
      const result = await updateComponentCategory(draggedId, { parentId: targetId });
      if (result.success) {
        toast({ title: "移动成功" });
        router.refresh();
      } else {
  ...
  const createFields: FieldConfig[] = [
    ...
    { key: "name", label: "分类名称", type: "text", placeholder: "如：内存" },
  ];
  ...
  const handleEdit = async (category: Category, values: Record<string, string>) => {
    const result = await updateComponentCategory(category.id, { name: values.name });
    if (result.success) {
      router.refresh();
    }
    return result;
